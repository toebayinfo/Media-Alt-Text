...
if (! class_exists('Media_Alt_Text_Enhancer')) {
    class Media_Alt_Text_Enhancer
    {
        ...
        private function handle_generation_request(bool $resume_failed): array
        {
            ...
            $retry_count = 0;
            if ($resume_failed) {
                $state       = $this->get_failed_run_state();
                $attachments = array_map('intval', $state['ids'] ?? []);
                ...
                if (empty($attachments)) {
                    ...
                    return [
                        ...
                        'skipped' => 0,
                        'failed'  => 0,
                        'retried' => intval($state['retries'] ?? 0),
                        'errors'  => [
                            __('No failed attachments remain to resume.', 'media-alt-text-enhancer'),
                        ],
                    ];
                }

                $retry_count = intval($state['retries'] ?? 0) + 1;
            } else {
            ...
            $this->persist_failed_run_state($results['failed_ids'], $results['failed'] > 0 ? $retry_count : 0);

            unset($results['failed_ids']);
